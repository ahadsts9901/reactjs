...
app.get('/api/v1/products', (req, res) => {
    ...
    try {
        if (search) {
            const filtered_products = products.filter(product => (product.description.includes(search) || product.title.includes(search)))
            return res.send({ message: "products fetched", products: filtered_products })
        }
